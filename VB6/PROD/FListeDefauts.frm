...
Begin VB.Form FListeDefauts 
   ...
   Begin VB.PictureBox PBBoutons 
      ...
      Begin VB.CommandButton CBPasSignalisationDefauts 
         ...
         Style           =   1  'Graphical
         TabIndex        =   2
         ToolTipText     =   " Pas de signalisation des défauts du chargement "
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   3015
      End
      Begin VB.CommandButton CBSignalerTousDefauts 
         BackColor       =   &H00FFFFFF&
         Caption         =   "Signaler tous les défauts"
         ...
         Style           =   1  'Graphical
         TabIndex        =   1
         ToolTipText     =   " Signale tous les défauts du chargement "
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   3015
      End
      Begin VB.CommandButton CBTraçabiliteAlarmes 
         BackColor       =   &H00FFFFFF&
         Caption         =   "Traçabilité des alarmes"
         ...
         Style           =   1  'Graphical
         TabIndex        =   0
         ToolTipText     =   " Traçabilité totale des alarmes du chargement "
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   3015
      End
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' Rôle                    : Fenêtre affichant la liste de l'ensemble des défauts possibles sur la ligne
...
Private Sub CBSignalerTousDefauts_Click()
    ...
    '--- signalisation de tous les défauts ---
...
Private Sub CBTraçabiliteAlarmes_Click()
    On Error Resume Next
    AppelFenetre F_TRACABILITE_ALARMES
End Sub

Private Sub CBTraçabiliteAlarmes_GotFocus()
...
Private Sub CBTraçabiliteAlarmes_LostFocus()
    On Error Resume Next
    SFocus.Visible = False
End Sub
...
Private Sub PBBoutons_Resize()
    ...
    CBTraçabiliteAlarmes.Left = CBSignalerTousDefauts.Left - MARGES.M_ENTRE_BOUTONS - CBTraçabiliteAlarmes.Width
...
Private Sub DechargeFenetre()
    ...
    '--- mise à jour de la grille pour éviter le message d'erreurs (data type mismatch during field update) ---
    TDBGListeDefauts.Update
    
    '--- réactualisation des défauts ---
    ChargeDefauts
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' Rôle      : Gestion de la liste des intervenants
    ...
    Select Case EtatSouhaite
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With TDBDDIntervenants
                ...
                .HeadBackColor = COULEURS.BLEU_4                      'couleur de fond des entêtes
                ...
                With .Styles(5)                                                             'couleur du curseur de sélection
                ...
                .AllowRowSizing = False                                              'interdire la modification de l'épaisseur des lignes
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' Rôle      : Gestion de la liste des défauts
' Entrées : EtatSouhaite -> Fonction de l'énumération GESTION_GRILLES
' Retours : "" indique aucun incident sinon le numéro de l'erreur est renvoyé
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Select Case EtatSouhaite
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With TDBGListeDefauts
                ...
                .AllowUpdate = True                                                       'autoriser la modification des données
                ...
                With .Columns(COLONNES_DETAILS_LISTE_DEFAUTS.C_SIGNALER_OUI_NON)
                    ...
                    .Caption = "Signaler le défaut (OUI/NON)"
                ...
                With .Columns(COLONNES_DETAILS_LISTE_DEFAUTS.C_LIBELLE_DEFAUT_AFFICHEUR)
                    .Locked = False
                    .ValueItems.Presentation = dbgNormal
                    .Caption = "Libellé pour l'afficheur"
                    .Width = EPAISSEUR_CARACTERE * 60
                    .HeadingStyle.Alignment = dbgCenter
                    .HeadingStyle.TransparentForegroundPicture = True
                    .HeadingStyle.ForegroundPicturePosition = dbgFPLeftOfText
...
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' Rôle      : Signaler ou pas dans la base de données la totalité des défauts
' Entrées : AvecSignalisation ->  TRUE = Signale tous les défauts
'                                                  FALSE = Signale ancun défaut
' Retours :
' ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
