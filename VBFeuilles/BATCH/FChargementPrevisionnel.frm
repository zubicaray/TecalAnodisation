   ...
   Begin VB.PictureBox PBDeplacementFenetre 
      ...
      Begin VB.PictureBox PBDeplacementFenetre 
         ...
         Begin C1SizerLibCtl.C1Tab CTOnglets 
            ...
            TabOutlineColor =   0
            FrontTabForeColor=   0
            Caption         =   "Chargement|Pr?visionnel"
            Align           =   0
            CurrTab         =   0
            FirstTab        =   0
            Style           =   1
            ...
            Begin VB.PictureBox PBOnglets 
               ...
               Begin VB.PictureBox PBPrevisionnel 
                  ...
                  Begin VB.CommandButton CBTransfererVersChargement 
                     BackColor       =   &H0080FF80&
                     Caption         =   "Transf?rer -> chargement + acc?s "
                     BeginProperty Font 
                        Name            =   "MS Sans Serif"
                        Size            =   9.75
                        Charset         =   0
            ...
            Begin VB.PictureBox PBOnglets 
               ...
               Begin VB.CommandButton CBTransfererVersPrevisionnel 
                  BackColor       =   &H0080FF80&
                  Caption         =   "Transf?rer -> pr?visionnel"
                  BeginProperty Font 
                     Name            =   "MS Sans Serif"
                     Size            =   9.75
                     Charset         =   0
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le                    : Fen?tre g?rant le chargement et le pr?visionnel
...
Private Const NBR_COLONNES_PREVISIONNEL  As Integer = 13
...
Private Enum COLONNES_DETAILS_CHARGES
    ...
    C_NBR_PIECES = 3                                    'nombre de pi?ces
    C_DESIGNATION = 4                                   'd?signation
    C_GAMME = 5
    C_MATIERE = 6
...
Private Enum COLONNES_DETAILS_REFERENCES_CLIENT
    C_NUM_LIGNES = 0
    C_NBR_PIECES = 1                                    'nombre de pi?ces
    C_REFERENCE_CLIENT = 2                       'r?f?rence donn?e par le client
End Enum
...
Private Enum COLONNES_PREVISIONNEL
    C_NUM_LIGNES = 0
    C_CHOIX_IA = 1                                          'meilleur choix pour l'entr?e dans la ligne (moteur d'inf?rence)
    C_NUM_COMMANDE_INTERNE = 2          'n? de commande interne
    C_NBR_REPARATIONS = 3                        'nombre de r?parations
    C_CODE_CLIENT = 4                                  'code du client
    C_NBR_PIECES = 5                                    'nombre de pi?ces
    C_DESIGNATION = 6                                   'd?signation
...
Private MemNumLigneDetailsCharges As Integer               'm?moire d'un n? de ligne dans la grille des d?tails des charges
Private MemNumColonneDetailsCharges As Integer           'm?moire d'un n? de colonne dans la grille des d?tails des charges
...
Private MemTempsReelAnodisationSecondes As Long      'm?morisation du temps r?el d'anodisation en secondes
Private MemTempsReelAnodisationTexte As String            'm?morisation du temps r?el d'anodisation en texte

Private NumBarreEnCours As Integer                                   'repr?sente un num?ro du montage en cours
Private ModeUouIEnCours As MODES_U_OU_I                   'mode U ou I en cours

'--- tableaux priv?s ---
'ATTENTION les tableaux TChargement et TPrevisionnel sont communs ? tout le programme
'afin d'?tre disponible par le moteur d'inf?rence
...
Private Sub EtuveTpsPoste_Change()
  ...
  If Not IsNumeric(EtuveTpsPoste.Text) Then
    MsgBox ("Rentrer une valeur num?rique")
...
Private Sub CBCalculerPrevisionnel_Click()
    ...
    '--- calcul du pr?visionnel avec affichage des choix ---
...
Private Sub CBChoixPosteAnodisationPrevisionnel_Click(Index As Integer)
    ...
    '--- affectation du num?ro de ligne de la grille ---
...
Private Sub CBCompensation_Click()
    ...
    '--- d?claration ---
    Dim TempsNickeCompense As Long          'temps d'anodisation compens?
...
Private Sub CBTransfererVersChargement_Click(Index As Integer)
    ...
    '--- d?claration ---
    Dim FicheVidePrevisionnel As VarPrevisionnel     'fiche vide pr?visionnel
    Dim LigneDepartDeplacement As Integer               'ligne de d?part du d?placement
    
    '--- affectation de la ligne de d?part du d?placement ---
    ...
    If LigneDepartDeplacement > 0 And LigneDepartDeplacement <= NBR_LIGNES_PREVISIONNEL Then
        ...
        '--- contr?le des valeurs ? la ligne point?e ---
        With TPrevisionnel(LigneDepartDeplacement)
            ...
            If .NumCommandeInterne > 0 And .CodeClient <> "" And .NbrPieces > 0 And .Designation <> "" And .NumGammeAnodisation <> "" Then
                ...
                '--- s?lection du n? de barre ---
                Call LNumBarres_Click(Index:=TPrevisionnel(LigneDepartDeplacement).NumBarre)
                
                '--- affectation du num?ro de gamme et chargement de la gamme dans la m?moire ---
    ...
    '--- changement de l'onglet en fonction du bouton cliqu? ---
    If Index = 1 Then
...
Private Sub IAutorisationChargement_DragOver(Source As Control, X As Single, Y As Single, State As Integer)
    ...
    Select Case Source.Name
        ...
        Case IAutorisationChargement.Name
            '--- agissement ? l'int?rieur du contr?le de d?part ---
            Select Case State
     
                Case 0
                    '--- 0 = Entre (le contr?le source entre dans la port?e de la cible) ---
                    Source.DragIcon = ILIcones.ListImages("fleche droite ombre").ExtractIcon
     
                Case 1
                    '--- 1 = Sort (le contr?le source sort de la port?e de la cible) ---
                    Source.DragIcon = ILIcones.ListImages("sens interdit").ExtractIcon
                
                Case 2
                    '--- 2 = Dessus (le contr?le source est pass? d'une ? ? une autre dans la cible) ---
...
Private Sub IAutorisationChargement_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    '--- contr?le du d?but du glisser/d?placer ---
...
Private Sub LNumBarres_Click(Index As Integer)
    ...
    If Index = 0 Then
        ...
        '--- RAZ du num?ro de barre en cours ---
        NumBarreEnCours = 0
            ...
            For Each OccLNumBarres In LNumBarres
                ...
                With OccLNumBarres
                    ...
                    If .Index = Index Then
                        ...
                        '--- affectation du num?ro de barre en cours ---
        ...
        For a = 1 To NBR_LIGNES_DETAILS_GAMMES_PRODUCTION
            ...
            With TChargement.TGammesAnodisation.TDetailsGammesAnodisation(a)
                ...
                If Trim(TZones(.NumZone).Codezone) = "C13 ? C16" Then
                    ...
                    '--- m?morisation des temps de l'anodisation ---
...
Private Sub MEBEditionDetailsCharges_Change()
    ...
    If InterdireEvenements = False Then
        ...
        '--- rendre invisible la liste des r?f?rences du client ---
        PBReferencesClient.Visible = False
        ...
        Select Case MemNumColonne
            ...
            Case COLONNES_DETAILS_CHARGES.C_NUM_COMMANDE_INTERNE
                ...
                Else
                    ...
                    '--- possibilit? de valider plusieurs fois le m?me num?ro de commande interne ---
        ...
        With MSHFGPrevisionnel
            ...
            Select Case .Col
                ...
                Case COLONNES_PREVISIONNEL.C_NUM_GAMME_ANODISATION
                    '--- num?ro de la gamme d'anodisation ---
                    MemNumLigne = .Row
                    MemNumColonne = .Col
...
Private Sub MEBEditionPrevisionnel_LostFocus()
    ...
    '--- d?claration ---
    Dim ChoixPosteAnodisation As CHOIX_POSTE_ANODISATION                        'choix du poste d'anodisation fonction de l'?num?ration
    Dim NumGammeAnodisation As String                                                              'num?ro d'une gamme d'anodisation
    Dim TexteComplet As String, _
            TexteSansMasque As String
        ...
        Select Case MemNumColonne
            ...
            Case COLONNES_PREVISIONNEL.C_NUM_GAMME_ANODISATION
                ...
                If IsNumeric(TexteSansMasque) = True Then
                    ...
                    '--- affectation du num?ro de gamme d'anodisation ---
                    NumGammeAnodisation = Right(String(6, "0") & TexteSansMasque, 6)
                    
                    '--- contr?le de l'existence de la gamme ---
                    If ExistenceGammesAnodisation(NumGammeAnodisation) = TROUVE Then
                        
                        '--- affectation du num?ro de gamme d'anodisation dans le pr?visionnel ---
                        TPrevisionnel(MemNumLigne).NumGammeAnodisation = NumGammeAnodisation
                        
                        '--- chargement de la gamme d'anodisation dans le pr?visionnel ---
...
Private Sub MSHFGDetailsCharges_KeyDown(KeyCode As Integer, Shift As Integer)
    On Error Resume Next
    Select Case KeyCode
        Case vbKeyDelete: EditionChargement vbKeyBack  'simule un retour arri?re (effacement)
        Case Else
    End Select
End Sub
...
Private Sub MSHFGDetailsCharges_KeyPress(KeyAscii As Integer)
    On Error Resume Next
    EditionChargement KeyAscii  'envoi de la touche frapp?e
End Sub
...
Private Sub MSHFGDetailsCharges_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    With MSHFGDetailsCharges
        ...
        '--- analyse en fonction du num?ro de ligne ---
        If NumLigne > 0 Then
            ...
            Select Case NumColonne
                ...
                Case COLONNES_DETAILS_CHARGES.C_NUM_COMMANDE_INTERNE
                    '--- n? de commande interne ---
                    ' il faut d?tecter le click dans l'image de la cellule
                        ...
                        If X >= XInferieurImageCellule And Y >= YInferieurImageCellule And _
                           X <= XSuperieurImageCellule And Y <= YSuperieurImageCellule Then
                            ...
                            With PBReferencesClient
                                ...
                                '--- affiche le contr?le liste au bon endroit (en dessous de la cellule) ---
                                ...
                                '--- m?morisation des valeurs de ligne et colonne ---
...
Private Sub MSHFGDetailsCharges_Scroll()
    ...
    '--- rendre invisible la liste des r?f?rences du client en cas de scrolling ---
...
Private Sub MSHFGDetailsReferencesClient_KeyDown(KeyCode As Integer, Shift As Integer)
    On Error Resume Next
    Select Case KeyCode
        Case vbKeyDelete: EditionDetailsReferencesClient vbKeyBack  'simule un retour arri?re (effacement)
        Case Else
    End Select
End Sub
...
Private Sub MSHFGDetailsReferencesClient_KeyPress(KeyAscii As Integer)
    On Error Resume Next
    EditionDetailsReferencesClient KeyAscii  'envoi de la touche frapp?e
End Sub
...
Private Sub MSHFGPrevisionnel_KeyDown(KeyCode As Integer, Shift As Integer)
    On Error Resume Next
    Select Case KeyCode
        Case vbKeyDelete: EditionPrevisionnel vbKeyBack  'simule un retour arri?re (effacement)
        Case Else
    End Select
End Sub
...
Private Sub MSHFGPrevisionnel_KeyPress(KeyAscii As Integer)
    On Error Resume Next
    EditionPrevisionnel KeyAscii  'envoi de la touche frapp?e
End Sub
...
Private Sub MSHFGPrevisionnel_Scroll()
    ...
    '--- rendre invisible la liste d'?dition en cas de scrolling ---
...
Private Sub PBCharges_DragDrop(Index As Integer, Source As Control, X As Single, Y As Single)
    ...
    Select Case Source.Name
        ...
        Case IAutorisationChargement.Name
            '--- d?pose d'une source venant de l'autorisation de chargement sur une des charges ---
             With TEtatsPostes(Index)
    ...
    Select Case Source.Name
        ...
        Case IAutorisationChargement.Name
             ...
             Select Case State
                 ...
                 Case 2
                      '--- 2 = Dessus (le contr?le source est pass? d'une position ? une autre dans la cible) ---
...
Private Sub IEtatsPostes_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If Button = vbLeftButton Then
        ...
        '****************************************************************************************************************
        '                                     forcer l'entr?e de la charge dans la ligne (clic gauche)
        '****************************************************************************************************************
        
        '--- affectation ---
        NumCharge = TEtatsPostes(Index).NumCharge
        
        '--- forcer l'entr?e de la charge dans la ligne ---
        If NumCharge >= CHARGES.C_NUM_MINI And NumCharge <= CHARGES.C_NUM_MAXI Then
            ...
            With TEtatsCharges(NumCharge)
                ...
                If .PtrZoneGammeAnodisation = 0 Then
                    ...
                    If AppelFenetre(F_MESSAGE, _
                                            ...
                                            "c|Voulez-vous r?ellement EFFECTUER cette ACTION ?", _
                                            TYPES_MESSAGES.T_ATTENTION, _
                                            TYPES_BOUTONS.T_OUI_NON, _
                                            EMPLACEMENT_FOCUS.E_SUR_NON) = vbYes Then
                
                        '--- pointer la premi?re zone ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Permet ou non l'utilisation des outils des grilles
' Entr?es :  Typegrille -> fonction de l'?num?ration TYPES_GRILLES
...
Private Sub UtilisationOutilsGrilles(ByVal TypeGrille As TYPES_GRILLES, _
    ...
    '--- autorisation ou non sur les outils concern?s ---
...
' Entr?es : OngletChoisie -> onglet choisie ? l'ouverture en fonction de l'?num?ration des ONGLETS
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Public Sub ParametrageFenetre(ByVal OngletChoisie As ONGLETS_CHARGEMENT_PREVISIONNEL)
    ...
    '--- modification de la forme du chargement pr?visionnel ---
    ...
    '--- s?lectionner les croix des options des ponts pour la petite vitesse ---
    ...
    '--- d?s?lectionner l'activation de l'air dans le bain de brillantange ---
    ...
    '--- visualisation des diff?rents ?tats du chargement pr?visionnel ---
    EtatsChargementPrevisionnel
    
    '--- calcul du pr?visionnel ---
    CalculPrevisionnel
...
Public Sub InitialisationFenetre()
    ...
    '--- valeur par d?faut du champ du temps r?el d'anodisation ---
    With MEBTempsReelAnodisation
        If .Text = "" Then
            .Text = "00:00:00"
        End If
    ...
    CBRechercherPar(ONGLETS_CHARGEMENT_PREVISIONNEL.O_CHARGEMENT).ListIndex = IDX_RECHERCHER_PAR.IDX_NUM_GAMME - 1      '-1 car l'index d?marre de 0
    CBRechercherPar(ONGLETS_CHARGEMENT_PREVISIONNEL.O_PREVISIONNEL).ListIndex = IDX_RECHERCHER_PAR.IDX_NUM_GAMME - 1     '-1 car l'index d?marre de 0
    InterdireEvenements = False
    Dim a As Integer
    '--
        ...
        If NumLigne >= 1 And NumLigne <= NBR_LIGNES_PREVISIONNEL Then
            ...
            With TPrevisionnel(NumLigne)
                ...
                If .NumCommandeInterne > 0 Then
                    ...
                    '--- affectation du num?ro de gamme ---
...
Private Sub TOBGestionGrilleChargement_ButtonClick(Index As Integer, ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        ...
        Case "InsererLigne"
            '--- ins?re une ligne dans une grille ---
...
Private Sub TOBGestionGrillePrevisionnel_ButtonClick(Index As Integer, ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        ...
        Case "EffacerGrille"
            ...
            '--- effacement de la table du pr?visionnel ---
            If AppelFenetre(F_MESSAGE, _
                                    TITRE_MESSAGES, _
                                    MESSAGE_301, _
                                    TYPES_MESSAGES.T_AVERTISSEMENT, _
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Construction d'un cadre en 3D pour l'esth?tique d'un image
' Entr?es : ObjetPictureBox -> Objet PictureBox ou l'on doit tracer le cadre 3D
'                      Epaisseur3D -> D?termine le nombre de lignes composant l'effet 3D
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Effacement complet du chargement avec un rafraichissement de l'?cran
' Entr?es :
' Retours :
' D?tails  :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Private Sub EffacementCompletChargement()
    ...
    '--- effacer le d?lai suppl?mentaire de stabilisation de la charge ---
    TBDelaiSupStabilisationCharge.Text = ""
    
End Sub

'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Lance le calcul du pr?visionnel avec l'affichage des choix
' Entr?es :
' Retours :
' D?tails  :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Private Sub CalculPrevisionnelAvecAffichageChoix()
    ...
    If SFocusTablePrevisionnel.Visible = False Or BoutonCalculerALeFocus = True Then
        ...
        '--- affichage du choix du pr?visionnel ---
        GestionPrevisionnel GG_COMPRESSION
        GestionPrevisionnel GG_AFFICHAGE
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Visualisation des diff?rents ?tats du chargement pr?visionnel
' Entr?es :
' Retours :
' D?tails  :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Private Sub EtatsChargementPrevisionnel()
    ...
    For a = POSTES.P_CHGT_1 To POSTES.P_CHGT_2
        ...
        With TEtatsPostes(a)
            ...
            Else
                ...
                '--- suppression de la croix de condamnation si n?cessaire ---
            ...
            Select Case .EtatsChariots
                ...
                Case ETATS_CHARIOTS.E_PRESENT_VERROUILLE
                    '--- chariot pr?sent et verrouill? (avec charge ou non) ---
                    If .NumCharge >= CHARGES.C_NUM_MINI And .NumCharge <= CHARGES.C_NUM_MAXI Then
                        
                        '--- affectation du num?ro de l'image ---
                        ...
                        '--- chariot verrouill? avec une charge ---
                        ...
                        '--- choix de l'affichage entre les num?ros de charges et les num?ros de barres ---
                        If ModeAffichageSynoptique = MA_NUM_CHARGES Then
                            
                            '--- affichage par les num?ros de charges ---
                        ...
                        Else
                            ...
                            '--- affichage par les num?ros de barres ---
    ...
    '--- contr?le de la validit? du chargement ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : V?rifie l'existence d'une commande interne dans une des tables
' Entr?es :
' Retours  : ExistenceCommandeInterne ->   TRUE = La commande interne existe d?j? dans la table
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Contr?le la validit? du chargement
' Entr?es :
...
Private Function ControleValiditeChargement() As Boolean
    ...
    '--- contr?le pour ?tre s?r d'avoir au moins une fiche ---
    ...
    '--- contr?le sur les autres fiches ---
    ...
    '--- contr?le sur le num?ro de la gamme d'anodisation ---
    ...
    '--- contr?le si la gamme est passable en ligne ---
    ...
    '--- contr?le d'un num?ro de barre diff?rent de z?ro ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Charge la gamme d'anodisation pour le chargement et affiche les redresseurs concern?s
...
Private Sub ChargeGammeAnodisationChargement(ByVal NumGamme As String)
    ...
    Dim NumZone As Integer                                          'num?ro de zone
    Dim NumPremierPoste As Integer                            'num?ro du premier poste
    ...
    If RechercheGammesAnodisation(NumGamme) = TROUVE Then
        ...
        '--- affichage des num?ros de barres ---
                ...
                If NumZone > 0 Then
                    ...
                    Select Case NumPremierPoste
                        ...
                        Case PREMIER_BAIN To POSTES.P_C12, POSTES.P_C17 To DERNIER_POSTE
                            '--- d?graissage ---
                            GammePassableEnLigne = True                    'Mont?e de la variable gamme passable en ligne
                        ...
                        Case POSTES.P_C13, POSTES.P_C14, POSTES.P_C15, POSTES.P_C16
                            ...
                            '--- transfert du temps de gamme et m?morisation de celui-ci ---
    ...
    Select Case EtatSouhaite
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With MSHFGDetailsCharges
                ...
                .Col = COLONNES_DETAILS_CHARGES.C_NUM_COMMANDE_INTERNE
                .ColWidth(.Col) = 12 * EPAISSEUR_CARACTERE: .Text = "Num?ro de pointage"
                ...
                .Col = COLONNES_DETAILS_CHARGES.C_NBR_PIECES
                .ColWidth(.Col) = 7 * EPAISSEUR_CARACTERE: .Text = "Nombre de pi?ces"
                .ColAlignment(.Col) = flexAlignRightCenter

                .Col = COLONNES_DETAILS_CHARGES.C_DESIGNATION
                .ColWidth(.Col) = 15 * EPAISSEUR_CARACTERE: .Text = "D?signation"
                .ColAlignment(.Col) = flexAlignLeftCenter
                ...
                .ColWidth(.Col) = 12 * EPAISSEUR_CARACTERE: .Text = "Mati?re"
                .ColAlignment(.Col) = flexAlignLeftCenter
                ...
                For a = LBound(TChargement.TDetailsCharges()) To UBound(TChargement.TDetailsCharges())
                    ...
                    Else
                        ...
                        If .Text <> CStr(TChargement.TDetailsCharges(a).NumCommandeInterne) Then
                            ...
                            '--- bouton du choix des r?f?rences du client ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Gestion des d?tails des r?f?rences des clients
' Entr?es : EtatSouhaite -> Fonction de l'?num?ration GESTION_GRILLES
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Select Case EtatSouhaite
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With MSHFGDetailsReferencesClient
                ...
                .Col = COLONNES_DETAILS_REFERENCES_CLIENT.C_NBR_PIECES
                .ColWidth(.Col) = 7 * EPAISSEUR_CARACTERE: .Text = "Nbr de pi?ces"
                .ColAlignment(.Col) = flexAlignRightCenter

                .Col = COLONNES_DETAILS_REFERENCES_CLIENT.C_REFERENCE_CLIENT
                .ColWidth(.Col) = 28.2 * EPAISSEUR_CARACTERE: .Text = "R?f?rence du client"
                .ColAlignment(.Col) = flexAlignLeftCenter
        ...
        Case GESTION_GRILLES.GG_TRANSFERT_DONNEES
            ...
            If MemNumLigneDetailsCharges > 0 And MemNumColonneDetailsCharges > 0 Then
                ...
                If NumCommandeInterne > 0 Then
                    ...
                    '--- construction du tableau des n? de lignes des r?f?rences clients ---
                    'le tableau est de la forme n? de ligne puis quantit? etc ...
                    ...
                    '
                    '            '--- recherche des num?ros pr?sents ---
                    '            If NumLignesReferencesClient <> "" Then
        ...
        Case GESTION_GRILLES.GG_MEMORISATION
            ...
            '--- m?morisation dans le chargement ---
            With TChargement.TDetailsCharges(MemNumLigneDetailsCharges)
                .NumLignesReferencesClient = NumLignesReferencesClient
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Permet l'?dition du chargement
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Permet l'?dition du pr?visionnel
' Entr?es : KeyAscii -> Code ASCII de la touche frapp?e
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Insertion d'une commande interne dans une des grilles
' Entr?es :             GrilleConcernee -> Fonction de l'?num?ration TYPES_GRILLES
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Ins?re une ligne sur l'une des grilles
' Entr?es : GrilleConcernee -> Fonction de l'?num?ration TYPES_GRILLES
' Retours :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Transfert les donn?es d'une grille vers l'autre grille
' Entr?es : GrilleDepart         -> Fonction de l'?num?ration TYPES_GRILLES
...
Private Sub TransfertDonneesEntreGrilles(ByVal GrilleDepart As TYPES_GRILLES, _
    ...
    Else
        ...
        '--- transfert des donn?es vers le chargement ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Supprime une ligne sur l'une des grilles
' Entr?es : GrilleConcernee -> Fonction de l'?num?ration TYPES_GRILLES
' Retours :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Introduit une charge dans la ligne (transfert des valeurs de chargement dans l'automate)
' Entr?es : NumPoste -> Num?ro du poste ou se trouve la charge
' Retours :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Private Sub IntroductionChargeAuChargement(ByVal NumPoste As Integer)
    ...
    If NumPoste >= POSTES.P_CHGT_1 Then 'And NumPoste <= POSTES.P_CHGT_2 Then
        ...
        If TEtatsPostes(NumPoste).NumCharge = 0 Then
            ...
            '--- recherche du prochain num?ro de charge ---
            NumCharge = ProchainNumeroCharge()
    
            '---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            
            '--- initialisation pour ?tre s?r de vider toutes les donn?es ---
            InitialisationCharge NumCharge
    
            '---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    
            '--- remplir la totalit? de la fiche ---
            With TEtatsCharges(NumCharge)
                ...
                '---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                Call LogCharges("************************************")
                Call LogCharges("num?ro de barre : " & .NumBarreInc)
                '--- transfert des d?tails de la charge ---
                ...
                '--- transfert du temps r?el d'anodisation ---
                ...
                If CBOptionsEtuve(1).Value = 1 Then
                    'on d?cale la zone de d?chargement une ligne plus haut
                    ...
                    ' l'avant derni?re zone devient un passage ? l'?tuve
                ...
                If FRedresseurs.Visible = True Then
                    ...
                    '--- temps des phases, tension et intensit? ---
            ...
            If PROGRAMME_AVEC_AUTOMATE = True Then
                ...
                If EnvoiNumeroChargePosteAvecOptions(NumPoste, NumCharge, TEtatsCharges(NumCharge).Options1, TEtatsCharges(NumCharge).Options2) = OK Then
                    ...
                    '--- lancer l'affichage des donn?es transmises (compteur) ---
                    ...
                    '--- rendre invisible l'affichage des donn?es transmises (compteur) ---
                    ...
                    If EntreeAutomatiqueCharges = False Then
                        ...
                        TEtatsCharges(NumCharge).PtrZoneGammeAnodisation = 1             'pointer la premi?re zone pour indiquer un d?but de traitement
                                                                                                                                     '1 pointe normallement le chargement
                                                                                                                                     'uniquement en mode d'entr?es des charges en manuel,
                                                                                                                                     'si le mode d'entr?es des charges est sur automatique,
                                                                                                                                     'c 'est le moteur d'inf?rence qui force le pointeur ? 1
    ...
    Select Case EtatSouhaite
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With MSHFGPrevisionnel
                ...
                .ColWidth(.Col) = 27 * EPAISSEUR_CARACTERE: .Text = "D?signation"
                .ColAlignment(.Col) = flexAlignLeftCenter
                ...
                For a = LBound(TPrevisionnel()) To UBound(TPrevisionnel())
                    ...
                    Else
                        ...
                        Else
                            ...
                            '--- affichage de l'image du num?ro ---
...
' Entr?es :  NumOnglet  -> Num?ro de l'onglet fonction de l'?num?ration ONGLETS_CHARGEMENT_PREVISIONNEL
'                EtatSouhaite -> Fonction de l'?num?ration GESTION_GRILLES
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With TDBGGrilleRecherche(NumOnglet)
                ...
                If NumOnglet = O_CHARGEMENT Then
                    .HeadBackColor = COULEURS.ROUGE_5               'couleur de fond des ent?tes
                Else
