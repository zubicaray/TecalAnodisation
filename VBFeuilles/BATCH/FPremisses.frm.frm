...
Begin VB.Form FPremisses 
   ...
   Begin VB.PictureBox PBBoutons 
      ...
      Begin VB.CommandButton CBRegenerationComplete 
         ...
         Style           =   1  'Graphical
         TabIndex        =   6
         ToolTipText     =   " R?g?n?ration compl?te de toutes les pr?misses "
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   3255
      End
      ...
      Begin VB.CommandButton CBPremisseAutomatique 
         ...
         Style           =   1  'Graphical
         TabIndex        =   5
         ToolTipText     =   " G?n?ration de la pr?misse en automatique "
         Top             =   120
         UseMaskColor    =   -1  'True
         Width           =   3255
      End
   ...
   Begin VB.PictureBox PBPremisseGeree 
      ...
      Begin VB.Frame FChoixIA 
         BackColor       =   &H00FFFFFF&
         Caption         =   " Choix du moteur d'inf?rence "
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   9.75
            Charset         =   0
      ...
      Begin VB.Frame FDetailsPremisses 
         BackColor       =   &H00FFFFFF&
         Caption         =   " D?tails de la pr?misse "
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   9.75
            Charset         =   0
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le                    : Fen?tre g?rant les pr?misses
' Nom                    : FPremisses.frm
' Date de cr?ation : 20/12/2010
' D?tails                :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Effectue le lien entre les donn?es et les crit?res de recherche
' Entr?es : NumPosteDepart -> Poste de d?part
'                NumPosteArrivee -> Poste d'arriv?e
' Retours :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub CBPremisseAutomatique_Click()
    ...
    '--- calcul automatique de la pr?misse d?cod?e ---
   ...
   With TPremisses(NumPosteDepart, NumPosteArrivee)
    ...
    '--- transfert de la pr?misse dans le tableau ---
...
Private Sub CBSupprimer_Click()
    ...
    If AppelFenetre(F_MESSAGE, _
        ...
        '--- ATTENTION effacement des donn?es mais pas de l'enregistrement ---
        With ADODCPremisses.Recordset
...
Private Sub DGActions_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
    On Error Resume Next
    With DGActions 'pour fixer toujour la premi?re colonne
        .Col = 0
        .CurrentCellVisible = True
    End With
End Sub
...
' Entr?es : MethodeChoisie -> FALSE = Recherche
'                                                TRUE  = Tri
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Private Sub MSHFGDetailsPremisses_KeyDown(KeyCode As Integer, Shift As Integer)
    On Error Resume Next
    Select Case KeyCode
        Case vbKeyDelete: EditionDetailsPremisses vbKeyBack 'simule un retour arri?re (effacement)
        Case Else
    End Select
End Sub
...
Private Sub MSHFGDetailsPremisses_KeyPress(KeyAscii As Integer)
    On Error Resume Next
    EditionDetailsPremisses KeyAscii  'envoi de la touche frapp?e
End Sub
            ...
            Select Case .MouseCol
                ...
                Case COLONNES_DETAILS_PREMISSES.C_CODE_ACTION
                    ...
                    If TexteCellule <> "" And .MouseRow > 0 Then
                        .ToolTipText = " Num?ro de l'action = " & TDetailsPremisses(.MouseRow).NumAction & UN_ESPACE
                    End If
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Gestion des d?tails des pr?misses
    ...
    Select Case EtatSouhaite
        ...
        Case GESTION_GRILLES.GG_INITIALISATION
            ...
            With MSHFGDetailsPremisses
                ...
                .RowHeight(0) = 400                    '?paisseur des titres
                .Row = 0
                ...
                .Col = COLONNES_DETAILS_PREMISSES.C_PARAMETRE
                .ColWidth(.Col) = 10 * EPAISSEUR_CARACTERE: .Text = "Param?tre"
                .ColAlignment(.Col) = flexAlignCenterCenter
                
                .Col = COLONNES_DETAILS_PREMISSES.C_LIBELLE_ACTION
                .ColWidth(.Col) = 50 * EPAISSEUR_CARACTERE: .Text = "Libell? de l'action"
                                ...
                                Select Case TDetailsPremisses(a).CodeAction
                                    ...
                                    Case CODE_TEMPO_EGOUTTAGE
                                        ...
                                        .Text = "GAMME"                       'remplacement du texte par "GAMME"
                                                                                           'indiquant que le param?tre se trouve
                                                                                           'dans la gamme
                                    Case CODE_TEMPO_STABILISATION
                                        ...
                                        ' " + GAMME" indiquant que le d?lai suppl?mentaire de stabilisation est donn? au
                                        'chargement de la gamme
                                    
                                    Case Else
                                        '--- indiquer le poste quand le param?tre est un num?ro de poste ---
...
Private Sub InsertionDetail(Optional ByVal CodeAction As Variant)
    ...
    Else
        ...
        '--- le num?ro vient directement de la grille des d?tails ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Lecture de la partie concernant le moteur d'inf?rence dans les pr?misses
' Entr?es :
' Retours :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        With ADODCPremisses.Recordset
            ...
            If Not .BOF And Not .EOF Then
                ...
                If .status = adRecOK Then
                    ...
                    '--- extraction du n? du pont donn? par le moteur d'inf?rence et du temps de cycle par apprentissage ---
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Lecture des d?tails des pr?misses
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Permet l'?dition des d?tails d'une pr?misse
...
Public Sub InitialisationFenetre()
    ...
    '--- transfert des postes de d?part et arriv?e ---
    With CBNumPosteDepart
        .Clear
        .AddItem ("")
        .ItemData(.NewIndex) = 0
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : g?re le copier coller sp?cial
...
Public Sub GestionCopierCollerSpecial()
    ...
    Select Case NumFenetreEnCopie
        ...
        Case FENETRES.F_PREMISSES
            '--- pr?misses vers pr?misses (transfert de la pr?misse d?cod?e) ---
            With ADODCPremisses.Recordset
                If Not .BOF And Not .EOF Then
...
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
' R?le      : Construit les pr?misses (cod?e et d?cod?e)
' Entr?es :
' Retours :
'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
Private Sub ConstruitPremisses(ByRef PremisseCodee As String, _
    ...
    '--- ?limination du dernier s?parateur pour les deux pr?misses ---
        ...
        Case GESTION_GRILLES.GG_AFFICHAGE
            ...
            With TDBGGrilleRecherche
                ...
                With .Columns(COLONNES_GRILLE_RECHERCHE.C_NOM_POSTE_DEPART)
                    .Locked = True
                    .ValueItems.Presentation = dbgNormal
                    .Caption = "Nom du poste de d?part"
                    .Width = EPAISSEUR_CARACTERE * 10
                    .HeadingStyle.Alignment = dbgCenter
                    .HeadingStyle.TransparentForegroundPicture = True
                    .HeadingStyle.ForegroundPicturePosition = dbgFPLeftOfText
                ...
                With .Columns(COLONNES_GRILLE_RECHERCHE.C_NOM_POSTE_ARRIVEE)
                    .Locked = True
                    .ValueItems.Presentation = dbgNormal
                    .Caption = "Nom du poste d'arriv?e"
                    .Width = EPAISSEUR_CARACTERE * 10
                    .HeadingStyle.Alignment = dbgCenter
                    .HeadingStyle.TransparentForegroundPicture = True
                    .HeadingStyle.ForegroundPicturePosition = dbgFPLeftOfText
...
Private Sub TOBGestionGrilles_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim FicheVide As ImgDetailsPremisses              'fiche vide ? l'image des d?tails des pr?misses
